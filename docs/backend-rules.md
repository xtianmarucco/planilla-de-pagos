...
- The single connection variable is `DATABASE_URL`; never use the individual `DB_HOST / DB_PORT / DB_NAME / DB_USER / DB_PASSWORD` variables. Point it at Supabase's session-mode pooler (port 5432) so both the app and `prisma migrate deploy` work off the same connection.
