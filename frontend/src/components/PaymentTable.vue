<template>
  ...
  >
    ...
    >
      <td class="px-6 py-4 font-semibold text-brand-text text-sm">{{ payment.client.name }}</td>
      ...
      <td class="px-6 py-4">
        <span
          ...
          :class="{
            'bg-green-100 text-green-700':  payment.payment_method === 'EFECTIVO',
            'bg-primary/10 text-primary':   payment.payment_method === 'TRANSFERENCIA',
...
const formatDate = (val) =>
  new Date(val).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' });
</script>
