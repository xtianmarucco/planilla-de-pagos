    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <BaseCard v-for="stat in stats" :key="stat.label">
        <div class="flex items-start justify-between">
          <div class="space-y-3">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider m-0">{{ stat.label }}</p>
            <p class="text-3xl font-bold text-brand-text m-0">{{ stat.value }}</p>
          </div>
          <div
...
const totalCobradoFmt = computed(() =>
  new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(totalCobrado.value),
);
